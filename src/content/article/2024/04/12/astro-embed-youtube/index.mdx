...
しかし、AstroでYouTubeの通常の埋め込みコードを使用すると、ページの読み込み速度が低下してしまいます。YouTubeの埋め込みコードは速度を重視していないため、[PageSpeed Insights](https://pagespeed.web.dev/)などのツールで評価が低下してしまいます。
...
次に、YouTubeの動画を埋め込みたい箇所で``YouTube``コンポーネントを使います。すると、動画が埋め込まれます。
