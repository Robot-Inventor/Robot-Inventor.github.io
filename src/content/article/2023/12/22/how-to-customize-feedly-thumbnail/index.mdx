...
:::caution
`item.enclosure.length`は、本来はサムネイルのファイルサイズをバイト単位で指定します。AstroのRSSではこの値を省略できず、また`0`を指定するとエラーになるため、`1`を指定しています。

[RSS 2.0 Specification (Current)](https://www.rssboard.org/rss-specification#ltenclosuregtSubelementOfLtitemgt)では、`length`が省略可能とは書かれていないので、必須のようです。ただし、この値に`0`や`1`を指定しても、Feedlyでは問題なくサムネイルが表示されます。
:::
