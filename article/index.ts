...
if (article_container) article_container.appendChild(article_list_area);
else console.error("#article_container_innerが見つかりませんでした")

if (article_list_area) article_list_area.insertAdjacentElement("beforebegin", change_button);
else console.error("#article_list_areaが見つかりませんでした");
...
(async () => {
    ...
    if (window.innerHeight > window.innerWidth) change_card_type("landscape");
    else change_card_type("normal");
})();
